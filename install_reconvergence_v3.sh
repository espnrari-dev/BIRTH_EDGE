...
cat > reflection_reconcile_v3.py <<'PY'
#!/usr/bin/env python3
"""
BIRTH_EDGE RECONVERGENCE V3
...
- NO random values
- NO synthetic cases
- NO outcome leakage into model replay
- NO outcome leakage into wisdom replay
- ML parameters come only from persisted ml_model.json
- wisdom comes only from persisted wisdom_rules
- reality comes only from observed learning_results outcomes
- every reconstructed case contains provenance
"""
...
def extract_scale_params(
    ...
    """
    Supports common persisted scaling forms.
...
def rule_vote(
    ...
    """
    Conservative deterministic wisdom parser.

    A rule is applied ONLY when:

    1. Its text maps to a field actually present.
    2. Its threshold can be deterministically extracted.
    3. Its direction can be deterministically interpreted.

    Otherwise the rule is explicitly INAPPLICABLE.
    """
    ...
    if condition:
        vote = 0 if negative else 1
    else:
        vote = 1 if negative else 0
...
def observed_reality(
    ...
    pumped = (
        1 if pumped >= 0.5 else 0
    )

    rug_pulled = (
        1 if rug_pulled >= 0.5 else 0
    )
...
def main() -> int:
    ...
    for reflection in preserved:
        case_id = (
            reflection.get("case_id")
            or reflection.get("addr")
            or reflection.get("memory_id")
            or reflection.get("reflection_id")
        )
    ...
    for row in rows:
        ...
        if wisdom_result.get("available"):
            wisdom_available += 1
        ...
        if reality_result.get("available"):
            reality_available += 1
        ...
        if not wisdom_result.get("available"):
            missing_channels.append(
                {
                    "channel": "WISDOM",
        ...
        if not reality_result.get("available"):
            missing_channels.append(
                {
                    "channel": "REALITY",
        ...
        reflection = {
            ...
            "model_replay": {
                "raw_logit": (
                    model_result["raw_logit"]
                ),
                "feature_names": (
                    model_result["feature_names"]
                ),
                "scale_modes": (
                    model_result["scale_modes"]
                ),
            },
            "wisdom_replay": {
    ...
    audit = {
        ...
        "wisdom_rules": len(rules),
        "existing_reflection_count": len(old),
...
python3 -m py_compile reflection_reconcile_v3.py

echo
echo "=== RUNNING V3 DETERMINISTIC REPLAY ==="
python3 reflection_reconcile_v3.py

echo
echo "=== RECONVERGENCE AFTER V3 REPLAY ==="
python3 reconvergence.py

echo
echo "=== V3 AUDIT SUMMARY ==="
python3 - <<'PY'
import json
...
print(
    "synthetic_data:",
    audit.get("synthetic_data"),
)

print(
    "outcome_leakage:",
    audit.get("outcome_leakage"),
)
...
echo "V3 uses persisted model parameters, persisted wisdom rules,"
echo "and observed historical outcomes only."
