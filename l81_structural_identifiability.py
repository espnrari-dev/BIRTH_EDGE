...
Purpose
-------
Determine whether L8 RAW RULE FINGERPRINT INSTABILITY represents:

    1. genuine structural instability
    2. syntactic instability with structural stability
    3. functional stability despite different discovered rules

The miner and definitive architecture are NOT modified.
...
Important
---------
Exact string identity is intentionally separated from structural
and functional identity.

A rule may differ textually while representing substantially the
...
def flatten_and(expr):
    ...
    if op in ("AND", "&&") and children:
        ...
        for child in children:
            output.extend(flatten_and(child))
...
def predicate_signature(expr, module):
    ...
    Falls back to deterministic text when the expression object
    does not expose its internal predicate representation.
    """
    ...
    # Fallback.
    text = rule_string(expr, module).strip()
...
def canonical_feature_set(expr, module):
    ...
    # First use actual miner feature extraction.
    for feature in rule_features(expr, module):
        result.add(str(feature))

    # Then inspect canonical predicates.
    for feature, operator, threshold in canonical_predicates(
        expr,
        module,
    ):
...
def run():
    ...
    print(f"Common test N      : {COMMON_TEST_N}")
    ...
    for seed in SEEDS:
        ...
        # IMPORTANT:
        # Retain the actual expression so the common-test
        # phase can evaluate this exact discovered rule.
        item = {
            "seed": seed,
            "success": True,
    ...
    if common_accuracy:
        ...
        print(
            f"  Accuracy min        : "
            f"{min(common_accuracy):.6f}"
        )

        print(
            f"  Accuracy max        : "
            f"{max(common_accuracy):.6f}"
        )
    ...
    if common_balanced:
        ...
        print(
            f"  Balanced min        : "
            f"{min(common_balanced):.6f}"
        )

        print(
            f"  Balanced max        : "
            f"{max(common_balanced):.6f}"
        )
    ...
    report = {
        ...
        "method_note":
            (
                ...
                "structural Jaccard similarity, threshold "
                "stability, and functional prediction agreement. "
                "Discovery occurs once per seed. All discovered "
                "rules are evaluated on one common held-out "
                "dataset."
            ),
